...
interface GnodProps extends SvgProps {
  /** Primary color for the main gnome shape */
  primaryColor?: string
  /** Secondary color for the hand/body parts */
  secondaryColor?: string
  /** Whether the icon is in a "liked" state (affects colors) */
  isLiked?: boolean
  /** Predefined color scheme when liked */
  likedPrimaryColor?: string
  likedSecondaryColor?: string
  /** Predefined color scheme when not liked */
  unlikedPrimaryColor?: string
  unlikedSecondaryColor?: string
}
...
function Gnod({
  ...
  secondaryColor,
  isLiked = false,
  likedPrimaryColor = '#15803d', // Green from theme
  likedSecondaryColor = '#166534', // Darker green
  unlikedPrimaryColor = '#9ca3af', // Gray
  unlikedSecondaryColor = '#6b7280', // Darker gray
  ...props
}: GnodProps) {
  // Determine colors based on state or explicit props
  const mainColor = primaryColor || (isLiked ? likedPrimaryColor : unlikedPrimaryColor)
  const accentColor = secondaryColor || (isLiked ? likedSecondaryColor : unlikedSecondaryColor)

  return (
    <Svg width={props.width || 16} height={props.height || 18} viewBox="0 0 37.17128 43.839218" {...props}>
      <G transform="translate(-33.242 -288.7)">
        {/* Main gnome head outline */}
        <Path
